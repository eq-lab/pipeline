/**
 * `useBlendDeposit` — write hook for supplying collateral to the Blend pool.
 *
 * Returns `{ write(amount, reserveId?), data, isPending, isSuccess, error, reset }`.
 *
 * Mock layer: when `pipeline.mock.wallet.stellar.blend.deposit` is set in
 * localStorage, `write()` resolves with the mocked `{ hash }` without
 * touching the Soroban RPC or the wallet signing flow.
 *
 * Real path: builds the `SupplyCollateral` submit op, runs the full Soroban
...
/**
 * Write hook for `SupplyCollateral` (deposit) operations on the Blend pool.
 *
 ...
 */
export function useBlendDeposit(defaultReserveId?: string): BlendWriteResult {
