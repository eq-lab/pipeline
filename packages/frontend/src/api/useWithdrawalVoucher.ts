 ...
 * Pipeline API (`GET /v1/withdrawals/{request_id}/voucher?wallet=<address>`).
 *
 * The voucher contains the `signature` needed to call `useClaimWithdrawal.write()`.
...
export interface WithdrawalVoucherResponse {
  request_id: string;
  amount: string;
  user: string;
  /** Verifier signature — passed directly to `useClaimWithdrawal.write()`. */
  signature: string;
}

export type WithdrawalVoucherStatus = "idle" | "pending" | "ready" | "failed";

export interface UseWithdrawalVoucherResult {
  data: WithdrawalVoucherResponse | undefined;
  status: WithdrawalVoucherStatus;
  error: Error | null;
  refetch: () => void;
}
...
/**
 * Returns the withdrawal voucher for the given request ID.
 *
 ...
 *
 * @param requestId  The withdrawal request ID (string, e.g. "77"). Pass `undefined`
 *                   to keep the hook in the idle/disabled state.
 */
export function useWithdrawalVoucher(
  ...
  const query = useQuery<WithdrawalVoucherResponse, Error>({
    queryKey: ["withdrawal-voucher", requestId, address, mockVer],
    queryFn: () =>
      apiFetch<WithdrawalVoucherResponse>(
        `/v1/withdrawals/${requestId}/voucher?wallet=${address ?? ""}`,
      ),
    enabled,
  ...
  let status: WithdrawalVoucherStatus;
  if (!enabled) {
    status = "idle";
