...
/// A transfer that has been Trustee-approved (#936) — only relevant for on-ramp
/// (ramp→custody) events, which `compute_capital_allocation`'s `back` leg now only
/// nets out when approved.
...
fn in_transit_ignores_unapproved_on_ramp() {
    ...
    // an unapproved on-ramp transfer must not reduce in_transit at all.
    let transfers = vec![
        transfer(CUSTODY, RAMP, 100), // out: +100
        transfer(RAMP, CUSTODY, 30),  // back, NOT approved: ignored
    ];
    let sets = addr_sets();
    let r = compute_capital_allocation(&[], &[], 0, &transfers, Some(&sets), &BigDecimal::from(0));
    assert_eq!(r.buckets.in_transit, Some("100.000000".to_owned()));
