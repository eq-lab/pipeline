...
// USDC and sPLUSD render from real vector SVGs via Vite's `?url` import,
// matching the approach used for HeroIcon (Issue #238).
// PLUSD still uses a rasterised base64 PNG (pending a follow-up cleanup).
 ...
 *
 * USDC and sPLUSD render from vector SVG assets (Issue #246, #534). PLUSD still
 * uses a rasterised PNG sprite embedded as a base64 data URI (pending cleanup).
 *
...
// sPLUSD now renders from a real vector SVG (Figma node 910:10323, issue #534).
...
export const CoinIcon = React.forwardRef<HTMLImageElement, CoinIconProps>(
  function CoinIcon(
    ...
    const src =
      isUsdc ? usdcSrc
      : isSplusd ? splusdSrc
      : `data:image/png;base64,${PLUSD_B64}`;
