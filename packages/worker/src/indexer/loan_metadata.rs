...
// The off-chain metadata document type and its fetcher now live in `shared` so the
// API can validate a submission's `metadata_uri` against the exact type the indexer
// parses. Re-exported here to keep existing `crate::indexer::loan_metadata::…` imports working.
