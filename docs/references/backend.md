...
- `JOB_INDEXER_STELLAR_ASSET_ID` — Soroban `C…` id of the asset (SAC) to watch. Set alone, this is **sufficient to enable tracking** for any chain that also configures a `CHAIN_<id>_STELLAR_WITHDRAWAL_QUEUE_WALLET_ID` (see below). Unset → the whole feature ships dark.
- `CHAIN_<id>_STELLAR_WITHDRAWAL_QUEUE_WALLET_ID` — per-chain, optional. The **Withdrawal Queue Wallet**: a separate MPC custody `G…` account that actually holds the USDC for queue settlement (see [withdrawals.md](../product-specs/withdrawals.md)) — distinct from `CHAIN_<id>_STELLAR_WITHDRAWAL_QUEUE_ID`, which is just the accounting contract and never itself holds a USDC balance. When set (with `ASSET_ID` also set), every transfer where `from` or `to` is this wallet is stored unconditionally — one-sided, any counterparty (e.g. USDC moving between the Withdrawal Queue Wallet and an arbitrary LP wallet).
...
**`GET /v1/positions/history`.** Position history for a wallet, reading `position_events` via `PositionRepo::get_position_history`. The period surface mirrors `/v1/stats/prices` — `days` (omit for all history), `interval` (`hourly` | `daily` | `weekly`, default `daily`), `chain_id` — plus `wallet` (required). The shared `intervals::MAX_SAMPLES` cap (1_000 buckets) applies, rejecting over-wide requests with a 400 that names the estimate.

It takes **no `vault` parameter**: the protocol runs a single vault (sPLUSD), so `chain_id` identifies it. The response is a flat series — `{ wallet, vault_address, interval, history[] }` — matching the shape of `/v1/stats/prices`.
