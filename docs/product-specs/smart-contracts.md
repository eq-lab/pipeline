...
| WHITELIST_ADMIN | WhitelistRegistry | DepositManager proxy | ADMIN 3/5 | GUARDIAN 2/5 |
| WHITELIST_REVOKER | WhitelistRegistry | Relayer EOA | ADMIN 3/5 | GUARDIAN 2/5 |
...
compromised actor). Operational roles held by EOAs (`WHITELIST_REVOKER`, `TRUSTEE`,
and the Relayer-side signing keys referenced by `YieldMinter`, `DepositManager`,
`WithdrawalQueue`, and `WhitelistRegistry` as configured addresses) are the ones GUARDIAN
revokes in a compromise (for the role) or ADMIN rotates (for the signing keys).
...
- **Granular defensive response.** GUARDIAN's emergency toolkit is instant but narrow:
  ...
  of a named operational role (`WHITELIST_REVOKER`, `TRUSTEE`). There is no
