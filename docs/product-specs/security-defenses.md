...
### Layer 1, DepositManager (two-step screened deposit)

DepositManager closes the Resolv attack class for the deposit leg. Users call
`deposit(usdcAmount)`, which pulls USDC from the LP via
`transferFrom(lp, intakeWallet, amount)` and creates a `Pending` ticket. The Relayer
runs KYT off-chain. On a clean result, the Relayer signs an EIP-712 `ClaimAttestation`
and serves it via API (no on-chain write). The lender calls
`claim(depositId, attestation, signature)`, which:

- Verifies the signature on-chain against the configured `kytAttestor` address.
- Calls `WhitelistRegistry.setAccess(lender, att.approvedAt)` to enrol the lender.
- Pulls USDC from the Intake Wallet to the Capital Wallet via standing allowance.
- Calls `PLUSD.mintForDeposit(lp, amount)` (restricted to the `DEPOSITOR` role held by
  DepositManager).
- The PLUSD `_update` hook enforces the recipient is whitelisted, which the auto-enrol
  write in step 2 guarantees.

PLUSD is never minted without the lender's USDC having arrived in the Intake Wallet
first. A Relayer compromise can sign valid attestations without a real KYT pass but
cannot mint PLUSD on its own (no real ticket means no claim path). The risk of Relayer
compromise is AML (illicit USDC entering the Capital Wallet on a real deposit), not
direct theft. MINT_ATTESTOR is retired.
...
- `AccessManager.revokeRole(role, account)` for operational roles only
  (`YIELD_MINTER`, `WHITELIST_REVOKER`, `TRUSTEE`), one role, one holder at
  a time.
...
| Revoke operational-role holder (`YIELD_MINTER`, `WHITELIST_REVOKER`, `TRUSTEE`) | GUARDIAN 2/5 | instant | — |
...
| Rotate `YIELD_MINTER` / `WHITELIST_REVOKER` / `kytAttestor` / `relayerYieldAttestor` | ADMIN 3/5 | 48h | GUARDIAN |
...
| WithdrawalQueue | `requestWithdrawal`, `claim`, `claimAtShutdown` revert. | Deposits, stake/unstake. |
