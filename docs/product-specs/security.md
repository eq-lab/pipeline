...
|---|---|---|
| **Relayer operational key (`kytAttestor` signing key + `WHITELIST_REVOKER` role + `relayerYieldAttestor` signing key)** | Cannot mint deposit PLUSD on its own. `claim` requires the lender's USDC to have already arrived in the Intake Wallet (which requires a real prior `deposit` call from the lender), plus a valid `ClaimAttestation` signed by `kytAttestor`. A compromised key can sign valid attestations for any depositId, but PLUSD is only minted when the lender themselves calls `claim`. Cannot mint yield alone (`YieldMinter.mintLoanYield` / `mintTbillYield` require a valid custodian EIP-1271 signature). Can revoke whitelist entries via `WHITELIST_REVOKER`, but whitelist enrolment lands only via DepositManager.claim or holder-submitted `enrol`. | Bounded. ADMIN rotates the signing keys via `setKytAttestor` and `proposeYieldAttestors` under 48h timelock. GUARDIAN can instantly pause PLUSD/DepositManager/WQ and revoke `WHITELIST_REVOKER`. No unbacked mint, no direct theft. The risk is AML (illicit USDC entering the Capital Wallet via a bypassed KYT on a real deposit, illicit addresses gaining transfer eligibility through a forged attestation submitted by a colluding holder). |
| **Relayer yield-signer key (`relayerYieldAttestor`) alone** | Can produce valid Relayer-side EIP-712 signatures for yield attestations. | Bounded — zero mints. The custodian's EIP-1271 signer independently verifies the underlying USDC inflow and signs second; YieldMinter verifies both on-chain. Compromising Relayer's yield signer alone mints nothing. 48h-timelocked rotation via `proposeYieldAttestors`. |
...
| **Trustee key** | Can write any Trustee-branch LoanRegistry state: mint ghost loans, write false `recordPayment` entries, `rollover` a loan, `closeLoan(ScheduledMaturity)` without an actual repayment. Is also one cosigner on the Capital Wallet MPC. | Data-integrity damage only on the on-chain LoanRegistry — it has no capital touchpoints and is not a NAV source, so false entries do not inflate share price or drain funds. A false `recordPayment` or `rollover` still mints nothing on its own: the YieldMinter loan path additionally requires the two-party Relayer + custodian attestation and is bounded by the per-loan maturity-capped cap. Capital Wallet releases require Relayer cosign too, so single-key Trustee compromise cannot move USDC. `Default` and `amendEconomics` require RISK_COUNCIL (24h timelock) — unreachable by Trustee. GUARDIAN revokes `TRUSTEE` instantly; ADMIN rotates under 48h timelock. |
| **WHITELIST_ADMIN in isolation** | Can add any address to the whitelist. | Not a drain path on its own. Whitelisting an address grants transfer eligibility but does not mint. Mints require the address to call `claim` against a `Claimable` ticket the address itself created via `deposit` (with USDC moved to the Intake Wallet first), or the address to be the destination of a two-party-attested yield mint. |
| **kytAttestor signing key in isolation** | Can sign valid `ClaimAttestation` and `EnrolAttestation` payloads. | Not a drain path. A signed `ClaimAttestation` only enables the lender to call `claim` against their own already-deposited USDC. The result is a real PLUSD mint backed 1:1 by USDC the lender themselves moved into the Intake Wallet. A signed `EnrolAttestation` only enables an address holder to enrol themselves in the transfer whitelist. The damage is AML (KYT bypass on a real deposit, or transfer eligibility for a sanctioned address), not theft. |
...
| **Trustee + Relayer yield-signer joint compromise** | Trustee fabricates a `PaymentRecorded` event; the joint party produces a valid Relayer-side loan-yield attestation; custodian is asked to co-sign. | Custodian's EIP-1271 signer must independently verify the underlying USDC inflow against the custodian's own ledger; if no USDC actually arrived, the co-sign is refused. The YieldMinter per-loan cap also bounds the mint to the loan's maturity-capped accrual. Additional defence: Watchdog cross-checks `PaymentRecorded` against Capital Wallet inflows. |
...
| **Reentrancy on mint/burn paths** | Re-enter `mintForYield` or `mintForDeposit` inside a callback during `_mint`, before counters are updated. | Counter increments happen before `_mint`; all entry points carry `nonReentrant`. PLUSD is non-transferable and has no external callback in its transfer path. |
...
| **LP sanctioned post-mint** | LP holds PLUSD and is subsequently flagged by passive KYT re-screening. | Relayer calls `WhitelistRegistry.revokeAccess(lp)`. Subsequent `requestWithdrawal` reverts at PLUSD `_update` (transfer to queue rejected). If the LP already has a `Pending` queue entry, the `claim` call reverts at the queue's `isAllowed` re-check. ADMIN takes disposition via `adminRelease`. PLUSD already held is frozen from further transfers by the whitelist rule (every transfer requires both endpoints whitelisted or system). |
...
- **LoanRegistry mutable state is Trustee-attested, not on-chain verified.**
  `updateMutable`, `recordPayment`, `rollover`, and `closeLoan(ScheduledMaturity |
  EarlyRepayment)` are written on Trustee attestation only. `setDefault` and
  `amendEconomics` are the RISK_COUNCIL-gated exceptions. Because LoanRegistry has no
  capital touchpoints and is not a NAV source, this is data-integrity risk only, not fund
  risk.
- **90-day KYT freshness on the transfer whitelist.** `WhitelistRegistry.isAllowed`
  fails if `(block.timestamp - approvedAt) >= freshnessWindow`, even for whitelisted addresses.
  An expired entry blocks PLUSD and sPLUSD transfers until refreshed.
...
- **Addresses can exist in multiple allowlist categories simultaneously** (system,
  DeFi venue, screened lender). By design. Some addresses legitimately satisfy more than
  one category.
...
- **Trustee attestations over LoanRegistry mutable state.** `updateMutable`,
  `recordPayment`, `rollover`, and `closeLoan(ScheduledMaturity | EarlyRepayment)` are
  signed off-chain by the Trustee and written on-chain via the `TRUSTEE` role. The contract
  does not verify these attestations against capital movement. Mitigation: LoanRegistry
  has no capital touchpoints — data-integrity risk only, not fund risk. The YieldMinter
  per-loan cap bounds any mint regardless. Watchdog reconciles against Capital Wallet
  inflows off-chain.
