...
|---|---|---|---|---|---|---|
| 1 | Origination approval | Originator submits an EIP-712 signed request, Relayer validates the signature and queues it | Full immutable economics, the validated Originator signature, submission timestamp, resolved descriptive material (borrower hash, commodity, corridor, governing law, protection instrument, initial location). See flow note B | Approve, Request changes (comment), or Reject (reason) | `mintLoan(originator, economics, metadataURI, initialLocation)`. Mints the loan NFT, seeds `epochs[0]`, emits `LoanMinted`, which triggers Relayer disbursement prep (Flow 7) | `senior + equity == facilitySize`, `offtakerPrice >= facilitySize`, `maturity > origination`. Show the failing invariant before enabling broadcast |
| 2 | Repayment intake | The dashboard flags loans at or past a scheduled coupon or maturity with no matching `recordPayment` yet (repayment-due worklist) and notifies the Trustee. Bank integration is excluded from MVP, so the Trustee reconciles the prompt against the correspondent bank wire manually, then opens intake against the loan | Loan picker of open loans. On loan select: outstanding senior principal, actual tenor, protocol fee schedule, the auto-computed waterfall split, deviations from baseline flagged. See flow note A | Confirm received amount and date. Adjust individual components for waivers, partials, or early-repayment fees. The amount input accepts display-formatted values (`$`, `,` separators are stripped on parse) and renders comma-grouped as-you-type; a non-blocking warning (message + icon below the input) fires when the entered amount exceeds the served `offtaker_outstanding` (#1048) | `recordPayment(loanId, offtakerAmount, seniorPrincipal, seniorInterest, mgmtFee, perfFee, oetAlloc, equityAmount)`. The registry entry lands first as pure accounting: increments the seven per-loan counters, emits `PaymentRecorded`, moves no USDC, mints no PLUSD. The Trustee then instructs the USD-to-USDC on-ramp into the Capital Wallet, and the Relayer matches that inflow to mint (Flow 13). Both this on-ramp inflow and its later off-ramp counterpart are indexed as pending ramp events (`GET /v1/ramp/events`) that the Trustee must approve or reject with a reason (`POST /v1/ramp/events/{id}/review`) before Capital Allocation's `in_transit` figure counts either leg toward its gross flow — only an Approved decision counts. Once a loan's ramp movements settle, the Trustee records the confirmed per-loan amounts (and the loan's deployment flag and trust-account movements) via the full-upsert `POST /v1/loan-book/{loan_id}/transfers` (#1027) — these confirmed amounts are subtracted from the gross flow, so an unrecorded settlement leaves `in_transit` overstated and an over-recorded one drives it negative (deliberately unclamped) | Loan in {Performing, Watchlist}, the six split components sum `<= offtakerAmount`. Deferred interest is allowed (zero components). Recording is not minting |
...
against the `sum <= offtakerAmount` invariant before broadcast. **Shipped** as the baseline calculator `GET /v1/loan-book/{loan_id}/waterfall?amount=<raw base units>[&as_of][&chain_id]` (`packages/api/src/routes/waterfall.rs`): reads the loan snapshot (genesis rate, senior tranche, principal repaid) and the per-loan fee schedule (`loan_fee_schedule.{mgmt,perf,oet}_*_rate_bps`, migrations `20260717000001` + `20260720000001`), and returns five components — `senior_principal_returned`, `senior_coupon_net` (the net senior coupon → vault leg), `management_fee`, `performance_fee`, `oet_allocation` — each mapping 1:1 to a `recordPayment` argument. Money is raw on-chain base units (`recordPayment` scale, 7-decimal on Soroban); the baseline uses the genesis rate and origination→`as_of` tenor with no epoch folding.
...
**Implementation status (#821, #829, #831, #892):** a version ships at `/origination/$id` (reached from the "Review" control on the Origination table #813 and the Overview Needs Attention section #818), rendering the submission's Loan Terms + Deal Details from `GET /v1/loan-book/submissions` plus an Origination decision status chip; Deal Details also renders Protection (`loan_data.protection`, `—` when absent) and a Location row (`loan_data.initial_location`, #1014) after Governing law, rendered `{location_type} — {location_identifier}` (e.g. "Warehouse — SGS bonded stockpile, Callao, Peru"; a missing half renders alone, both missing renders `—`); no collateral valuation is shown pre-mint (no on-chain `loan_id` to value against yet). The backend may return merged loan lifecycle statuses for approved submissions; Origination surfaces normalize any status other than `InReview`, `Approved`, or `Rejected` to `Approved`, because the resulting loan lifecycle belongs on Loans surfaces. Approve is now **chain-first**: it invokes `execute(target, "draw_loan", args, caller)` on the executor/access-control contract, **signed by the connected Trustee wallet** (`caller` == the trustee address, satisfying the contract's `require_auth` with one signature) — the backend is not the trustee and holds no minting key. A pre-submit `simulateTransaction` is the "verify the loan" step; only once the mint confirms does the app call the existing DB-only `POST /v1/loan-book/submissions/{id}/review {decision:"Approved"}`. A wallet rejection or any on-chain failure makes **no** review call — the submission stays `InReview`, retryable, with a mapped inline error (no signature is requested if the simulate step fails). Reject is unchanged: a reason dialog (min 5 chars trimmed) and `{decision:"Rejected",reason}`, a pure DB status flip. The Approved banner reads "Approved & minted · `<date>`". The Origination table itself shows **only in-flight originations** (#1044): submissions whose normalized status is `Approved` (including backend merged/lifecycle statuses) are filtered out — the resulting loan lives on the Loans surfaces — while `InReview`, `ChangesRequested`, and `Rejected` (the terminal decision record) remain; a direct URL to an approved submission's detail page still renders its Approved banner. **Known limitation:** a mint that succeeds but whose review call then fails leaves the loan minted on-chain while the submission stays `InReview` in the DB — reconciliation from the indexed `loan_drawn` event is a tracked backend follow-up, not yet built. "Request changes" is wired (#1017, backend #949): it opens a reason dialog (multi-line textarea, min 5 chars trimmed) and posts `{decision:"ChangesRequested",reason}` to the same review endpoint — a pure DB status flip, no mint, no wallet signature. `ChangesRequested` is non-final: the submission stays open (the originator can amend and resubmit; the backend also allows the trustee to review it again, though the UI currently renders that state banner-only per #950), and the footer flips to the Changes-requested banner on success. The Overview's **Needs Attention** (Origination group) lists `ChangesRequested` submissions alongside `InReview` (#1046, reversing the original #950-era exclusion): the row title reads "`<originator> — <commodity>: changes requested`" and its "Review" link opens the same `/origination/$id` detail (reason banner); `Rejected` stays out of Needs Attention — it is a terminal decision record with nothing left to act on. The section's data fetch is a single unfiltered `GET /v1/loan-book/submissions` filtered client-side to normalized `InReview`/`ChangesRequested` (the endpoint's `status` filter accepts one value per request).
...
| Loans page (`GET /v1/loan-book`) | `summary` = five tiles: **Deployed senior** (`deployed_senior`), **At-risk (WL + Default)** (`at_risk_wl_and_default_senior` + `at_risk_wl_and_default_pct` over NAV — WatchList/Default across all open loans, including defaulted/closed loans that are visible in `loans[]` but excluded from the active-set aggregates), **Weighted rate** (`weighted_rate`), **Weighted tenor** (`weighted_tenor_days`), **Top concentration** (`top_concentration`, largest commodity by senior share; limit frontend-owned). `loans[]` = table columns: commodity **spot price + 7-day change** (`spot_price` / `spot_change_7d`), **senior outstanding** (`senior_outstanding`), **senior original** (`original_senior_tranche`, deployed — distinct from outstanding and from `deployed_senior`), collateral, **CCR** (`ccr_bps`, off-chain computed) + on-chain **reported CCR** (`reported_ccr_bps`) + **report time** (`ccr_reported_at`), **maturity** (`maturity`, rollover-aware), **nearest payment** (`next_payment_timestamp`, the next scheduled payment — the current rollover-adjusted maturity for today's bullet loans, equal to `maturity`) + **days overdue** (`days_overdue`, whole days past `next_payment_timestamp`; `null` until then — pure date math, status-independent), status, **repaid to date** (`repaid_to_date`, cumulative offtaker cash received), **disbursed** (`disbursed`, off-ramp-complete flag), **protection** (`protection`, trade-finance protection instrument — rendered as a Protection row in the loan-detail Registry card, `—` when `null`, #1014), **days on watchlist** (`days_on_watchlist` + raw entry timestamp `watchlist_entered_at`, both `null` unless currently WatchList). `next_payment_timestamp` / `days_overdue` are also served per-loan on `GET /v1/loan-book/{loan_id}/financials`. `weighted_rate`/`weighted_tenor_days` mirror the Protocol-Dashboard `avg_yield`/`avg_duration_days`. | Surface 16 + Loans table |
