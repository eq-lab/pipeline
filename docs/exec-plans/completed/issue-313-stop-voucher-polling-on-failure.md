...
## Completion postscript (2026-08-28, PR #1220)

The plan's Open Question (also fix `useDepositVoucher`?) sat unanswered for three months; the user
directed proceeding on 2026-08-28 and the audit widened the answer: all **four** voucher hooks
(EVM + Stellar × deposit + withdrawal) shared the identical `refetchInterval` bug. Implemented as
one shared `isRetriableVoucherError` classification in `api/voucherRetry.ts` used by both `retry`
and `refetchInterval` in all four hooks, with predicate-matrix and polling-stop tests. Executed on
a fresh branch off main (`fix/313-voucher-polling-stop`); the original
`fix/313-stop-voucher-polling-non-retriable` branch is superseded and can be deleted.
