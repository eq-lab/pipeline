...
- **Resolved:** 2026-08-27 by #1003 — `NODE_OPTIONS=--no-experimental-webstorage` baked into the `test`/`test:watch` scripts of all three jsdom workspaces (frontend, wallet-connect, trustee); Node 20.19+ backported the experimental WebStorage global, so this hit pinned Node 20 too. All three suites green (1532 + 160 + 898); the two lingering `-deposit.test.tsx` toast failures were stale pre-#1142 title assertions, updated in the same PR. A `js-unit-tests` vitest job now runs the suites in CI.
- **Date:** 2026-08-24
