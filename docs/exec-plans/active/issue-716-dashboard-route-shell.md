...
- **Nav entry point already half-exists.** `MobileNavMenu.tsx` already renders a "Pipeline Overview" item (pie-chart glyph, node `1989:9444`) with **no route wired** (`onClick` is absent). This is the natural mobile entry for the Protocol Dashboard. The desktop `TopBar.tsx` has four nav slots (`home`/`deposit`/`stats`/`history`) and **no** overview slot. The mismatch between mobile and desktop nav is the main open question (below).
- **Figma frames are in a private file** — `WebFetch` cannot read them (returns only the word "Figma"). Exact grid columns, breakpoints, gaps, max-width, and panel order must be read from Figma at implementation time via the Figma MCP / browser, then encoded. The plan specifies a sensible default structure; the coder must reconcile it against the frames.
...
- Desktop navigation entry: should `/dashboard` get a **fifth nav slot in the desktop `TopBar`** (the bar currently shows only Home/Convert/Earn/Activity and has fixed 160px side slots that assume a centred 4-item nav), or is the Protocol Dashboard reached **only via the "Pipeline Overview" item** (currently mobile-menu only, no desktop equivalent)? The Figma desktop frame `3283-12098` should show the intended nav treatment — but because the frame is not machine-readable here, confirm with the design before adding/altering a TopBar slot. This affects `TopBar.tsx` layout (slot widths) and the active-nav derivation in both `TopBar.tsx` and `MobileNavMenu.tsx`.
...
6. **Wire the navigation entry point.** Wire the existing "Pipeline Overview" button in `MobileNavMenu.tsx` (currently no `onClick`) to navigate to `/dashboard` via the existing `handleNavClick` / `onNavigate` plumbing, and reflect active state when `pathname === "/dashboard"`. For desktop, implement the resolution of the **Open Question** above: either add a fifth slot to `NAV_ITEMS` in `TopBar.tsx` (+ matching active-nav derivation) or add a desktop overview affordance — exactly as the design dictates. Update the active-nav derivation in both `TopBar.tsx` and `MobileNavMenu.tsx` to handle `/dashboard`. If a new `NavIcon` is needed for an "overview" slot, check `packages/ui/src/components/NavIcon` for an existing glyph before adding one.
...
Update/extend `TopBar.test.tsx` and `MobileNavMenu`-related tests if the nav entry point changes active-state derivation (e.g. assert `/dashboard` marks the overview/dashboard slot active and that clicking it navigates).
