...
localStorage.setItem(
  ...
  JSON.stringify({
    ...
    amount: "10000000",
    user: "GBBD47...",
    signature: "a".repeat(128), // 64 bytes hex-encoded
  })
);
```
...
**Then:**
- `status` is `"ready"`
- `data.signature` is the 128-char hex string
...
Prerequisites:
1. Set `VITE_STELLAR_DEPOSIT_MANAGER_ID=CARFA2QETOZVKHSG4BCEEXMJHTYR2Z75VR7WQNX4MWZ33RQMKRKATIVI` in `.env`
